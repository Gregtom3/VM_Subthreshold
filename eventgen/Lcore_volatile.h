#ifndef _LCORE_VOLATILE_H_
#define _LCORE_VOLATILE_H_
...
namespace UPSILONMODEL{//Model of Upsilon (1S) production 
  ...
  double getB(const double Mv, const double Mp, const double W)
  {
    double s = W*W;
    double nu = 0.5*(s-Mp*Mp-Mv*Mv);
    double f = 0.238;
  ...
  double dSigmaY1S_v2(const double W, const double t)
  {
    ...
    _f1->SetParameters(tmin_,tmax_,sigma_el,A);
    double b = _f1->GetX(0.0,0.5,10); // units 1/GeV^2
    double b0 = getB(Mv,Mp,(Mv+Mp)*1.05);
    double jac = exp(-b0*t)/b0;
    return A*exp(b*t) * jac; // dsigma_dexp_b0t 
  }
...
namespace GENERATE{
  ...
  double Upsilon1SElectroproduction(TLorentzVector * ki, TLorentzVector *kf){
    ...
    double B = UPSILONMODEL::getB(Mup,mp,W);
    double b0 = UPSILONMODEL::getB(Mup,mp,(Mup+mp)*1.05);
