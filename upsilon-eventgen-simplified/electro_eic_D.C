...
int electro_eic_D(){
  ...
  // Beam Energies and Luminosities
  double Ebeam = 10;
  double Hbeam = 100;
  double lumi = 0.5 * 100 * pow(10,6); // 100fb^-1 -> nb^-1
                                       // Divide by # nucleons
  // Set nuclear
  ...
  GENERATE::Q2range[1] = 1.0;//GeV^2
  GENERATE::yrange[0]  = 0.01;
  GENERATE::yrange[1]  = 0.03;
  GENERATE::trange[0]  = -2.5;//GeV^2
  GENERATE::trange[1]  = 0.0;//GeV^2

  double volume = (GENERATE::Q2range[1] - GENERATE::Q2range[0])*(GENERATE::yrange[1]-GENERATE::yrange[0])*(GENERATE::trange[1]-GENERATE::trange[0]);

  // TTree Info
  TFile * fall2 = new TFile("result-eic-Y1S/eD2.root","RECREATE");
  ...
  tree->Branch("event", &myEvent.x,
	       "x/D:y/D:Q2/D:W/D:W2/D:t/D:x_true/D:y_true/D:Q2_true/D:W_true/D:W2_true/D:t_true/D:factor/D:factor_true/D:weight/D:weight_gamma_p/D:is_sub/I:is_accept/I");
  ...
  for (Long64_t i = 0; i < Nsim; i++){
	...
	myEvent.is_sub = 1;
      ...
      double factor = (GENERATE::yrange[1]-GENERATE::yrange[0])*(GENERATE::Q2range[1]-GENERATE::Q2range[0])*gammaT*(1.0+epsilon*R)*dipole;
      ...
      double factor_true = (GENERATE::yrange[1]-GENERATE::yrange[0])*(GENERATE::Q2range[1]-GENERATE::Q2range[0])*gammaT_true*(1.0+epsilon_true*R_true)*dipole_true;
      ...
      bool accept = (abs(eOut->Eta())<=5.0)&&(abs(ePlusOut->Eta())<=5.0)&&(abs(eMinusOut->Eta())<=5.0)&&(abs(pOut->Theta())>=0.002)&&(myEvent.y_true>0.01)&&(myEvent.y_true<0.8);
  ...
  double acceptance_rate = (1.0 * good_event)/(1.0 * Nsim);
  TH1D *h_phase_space = new TH1D("h_phase_space","",1,0,1);
  h_phase_space->Fill(0.5,acceptance_rate * volume);
  cout << "Acceptance Rate = " << good_event << " / " << Nsim << " = " << acceptance_rate << endl;
  cout << "Effective Phase Space = " << acceptance_rate * volume << endl;
  h_phase_space->Write();
  // Write the tree
  fall2->Write();
  fall2->Close();
