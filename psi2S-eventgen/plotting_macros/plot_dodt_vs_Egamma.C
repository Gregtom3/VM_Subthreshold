...
int plot_dodt_vs_Egamma()
{
  ...
  double Egamma_min = 12.1;
  double Egamma_max = 12.5;
  ...
  tIn2->Draw("-(event.t-tmin)>>h_2",Form("weight_smear2*(Eg_true>%f&&Eg_true<%f)",Egamma_min,Egamma_max),"goff");
  ...
  double text_left = 0.3;
  TCanvas *c0 = new TCanvas("c0","c0",800,600);
  tge[0]->Draw("AP");
  tge[0]->GetXaxis()->SetLimits(xmin,xmax);
  tge[0]->GetYaxis()->SetRangeUser(0.00001,1);
  latex.DrawLatexNDC(text_left,0.85,Form("#bf{SoLID} Simulation (%.1f GeV beam)",eIn_E));
  latex.DrawLatexNDC(text_left,0.8,Form("e+%s #rightarrow e'+p'+#psi(2S) (e^{-}e^{+}) , #color[4]{#bf{Photoprodution}}",stringA.Data()));
  latex.DrawLatexNDC(text_left,0.75,Form("%.2f GeV < E_{#gamma} < %.2f GeV",Egamma_min,Egamma_max));
  ...
  latex.DrawLatexNDC(text_left,0.85,Form("#bf{SoLID} Simulation (%.1f GeV beam)",eIn_E));
  latex.DrawLatexNDC(text_left,0.8,Form("e+%s #rightarrow e'+p'+#psi(2S) (e^{-}e^{+}) , #color[2]{#bf{Electroprodution}}",stringA.Data()));
  latex.DrawLatexNDC(text_left,0.75,Form("%.2f GeV < E_{#gamma} < %.2f GeV",Egamma_min,Egamma_max));
  ...
  latex.DrawLatexNDC(text_left,0.85,Form("#bf{SoLID} Simulation (%.1f GeV beam)",eIn_E));
  latex.DrawLatexNDC(text_left,0.8,Form("e+%s #rightarrow e'+p'+#psi(2S) (e^{-}e^{+}) , #color[882]{#bf{Photo+Electro prodution}}",stringA.Data()));
  latex.DrawLatexNDC(text_left,0.75,Form("%.2f GeV < E_{#gamma} < %.2f GeV",Egamma_min,Egamma_max));
